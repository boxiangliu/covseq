...
prefix = "/mnt/scratch/boxiang/projects/viraviz/src/vapid/EPI_ISL_402131/EPI_ISL_402126"
class Annotation():
    ...
    def merge_align_and_genbank(self):

        merged = self.nt_df
        protein_container = self.protein_container
        
        for protein in protein_container.values():
            merged = pd.merge(merged, protein["qry"], on="qry_coord", how="outer")
            merged = pd.merge(merged, protein["ref"], on="ref_coord", how="outer")

        self.anno_df = merged.sort_values("ref_coord")
    anno = Annotation(prefix)
prefix = "/mnt/scratch/boxiang/projects/viraviz/processed_data/parse_vapid/EPI_ISL_402126/EPI_ISL_402126"
class Annotation():
    def __init__(self, prefix):
        self.align_fn = f"{prefix}.ali"
        self.ref_fn = f"{prefix}_ref.fasta"
